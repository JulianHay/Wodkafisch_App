...
const FischGame = () => {
  ...
  const submitScore = async () => {
    ...
    try {
      const res = await client.post("/highscore", body, config);
      if (res.data.success) {
        setNotification("Highscore successfully saved");
        setScoreData({
          name: "",
          score: 0,
        });
  ...
  useEffect(() => {
    ...
    playerLeft.src = "/wodkafish-swim-left.png";
    ...
    enemyLeft.src = "/shark_swimming_left.png";
    const enemyRight = new Image();
    enemyRight.src = "/shark_swimming_right.png";
    const enemyLeftJaw = new Image();
    enemyLeftJaw.src = "/shark_swimming_left_jaw.png";
    ...
    class Enemy {
      ...
      update() {
        ...
        const y =
          player.x < this.x
            ? this.y - (this.height / 2) * tt * d
            : this.y + (this.height / 2) * tt * d;
        this.d_ellipse = Math.sqrt(
    ...
    function handleGameOver() {
      ...
      setScoreData({ ...scoreData, score: score });
      gameOver = true;
    ...
    function animate() {
      ...
      ctx.fillStyle = "rgba(34,147,214,1)"; //"rgb(32, 33, 60)"; //
      //   ctx.fillText("score: " + score, 10, 25);
      ctx.fillText("score: " + score, window.innerWidth * 0.5 - 100, 25);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Text text="Game Over!" fontSize={20} fontWeight="bold" />
          <Text
            text={`Your score: ${scoreData.score}`}
            fontSize={20}
            fontWeight="bold"
          />
          {modalVisible && scoreData.score > highscores[2].score ? (
            <>
              <Input
